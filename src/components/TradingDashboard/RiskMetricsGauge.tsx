    ...
    >
      <Card className="p-6 bg-card border-border">
        ...
        <p className="text-sm text-muted-foreground mb-6">
          Volatility, drawdown, and risk-adjusted return · {period}
        </p>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Circular gauges */}
          <div>
            <h4 className="text-xs font-medium text-muted-foreground mb-6 uppercase tracking-wide">
              Sharpe ratio analysis
            </h4>
            <div className="flex flex-wrap justify-center gap-8">
              <CircularGauge
                value={m.sharpeRatioMonthly}
                max={2}
                label="Monthly Sharpe"
                color="hsl(var(--primary))"
                description="Risk-adjusted return on a monthly basis"
                delay={0.1}
              />
              <CircularGauge
                value={m.sharpeRatioAnnualized}
                max={4}
                label="Annualised Sharpe"
                color="hsl(var(--success))"
                description="Above 2.0 is considered outstanding"
                delay={0.3}
              />
            </div>
          </div>
          ...
          <div className="space-y-5">
            ...
            <LinearGauge
              ...
              unit="%"
              color="hsl(var(--primary))"
              description="Portfolio return volatility — lower is more stable"
              delay={0.15}
            />
            <LinearGauge
              value={m.variance}
