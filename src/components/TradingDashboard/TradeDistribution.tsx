...
interface TradeDistributionProps { period?: Period; account?: AccountFilter; }
...
const getPeriodLabel = (period: Period, account: AccountFilter) => {
  if (account === "atlas")    return "3 Mar – 17 Apr 2026";
  if (account === "combined") return "28 Jan – 17 Apr 2026 · both accounts";
  return PERIOD_LABELS[period];
};

export const TradeDistribution = ({ period = "Q1", account = "equiti" }: TradeDistributionProps) => {
  const stats = getStats(period, account);
    ...
    >
      <Card className="p-4 md:p-6 bg-card border-border">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          {stats.totalTrades} closed trades · {getPeriodLabel(period, account)}
        </p>
        <div className="w-full h-[300px] md:h-[320px]">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart margin={{ top: 10, right: 10, bottom: 10, left: 10 }}>
              <Pie
                data={distributionData} cx="50%" cy="45%" outerRadius="55%"
                dataKey="value" isAnimationActive animationBegin={100} animationDuration={900}
      ...
      <Card className="p-4 md:p-6 bg-card border-border">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          Win / loss by direction · {getPeriodLabel(period, account)}
        </p>
        <div className="w-full h-[300px] md:h-[320px]">
          <ResponsiveContainer width="100%" height="100%">
