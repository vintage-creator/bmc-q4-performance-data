...
interface TradeDistributionProps { period?: Period; }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.4 }}
      ...
      <Card className="p-4 md:p-6 bg-card border-border">
        <h3 className="text-lg md:text-xl font-bold text-foreground mb-1">Win / loss distribution</h3>
        <p className="text-sm text-muted-foreground mb-4">{stats.totalTrades} closed trades · {periodLabel}</p>
        <div className="w-full h-[300px] md:h-[320px]">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart margin={{ top: 10, right: 10, bottom: 10, left: 10 }}>
              <Pie data={distributionData} cx="50%" cy="45%" outerRadius="55%" dataKey="value"
                ...
                label={({ cx, cy, midAngle, outerRadius, value, name }) => {
                  ...
                  return <text x={x} y={y} fill="hsl(var(--foreground))" textAnchor={x > cx ? "start" : "end"}
                    dominantBaseline="central" fontSize={12} fontWeight={500}>{name}: {value}</text>;
                }}>
                {distributionData.map((entry, i) => <Cell key={i} fill={entry.color} />)}
              </Pie>
              <Legend verticalAlign="bottom" height={36} iconType="circle" wrapperStyle={{ paddingTop: "12px", fontSize: "12px" }} />
      ...
      <Card className="p-4 md:p-6 bg-card border-border">
        <h3 className="text-lg md:text-xl font-bold text-foreground mb-1">Position type performance</h3>
        <p className="text-sm text-muted-foreground mb-4">Win / loss by direction · {periodLabel}</p>
        <div className="w-full h-[300px] md:h-[320px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={positionData} margin={{ top: 10, right: 10, bottom: 20, left: 0 }}>
              ...
              <Tooltip contentStyle={tooltipStyle} formatter={(v: number, name: string) => [v + " trades", name]} />
              <Legend verticalAlign="bottom" height={36} iconType="rect" wrapperStyle={{ paddingTop: "12px", fontSize: "12px" }} />
              <Bar dataKey="wins" stackId="a" fill="hsl(var(--success))" name="Wins" isAnimationActive animationDuration={800} />
              <Bar dataKey="losses" stackId="a" fill="hsl(var(--destructive))" name="Losses"
                isAnimationActive animationDuration={800} animationBegin={200} radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </Card>
