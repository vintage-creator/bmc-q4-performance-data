...
export const TradeStatsCard = ({ period = "Q1" }: TradeStatsCardProps) => {
  const s = statsMap[period];
  const winLossRatio =
    ...
    >
      <Card className="p-6 bg-card border-border">
        ...
        <p className="text-sm text-muted-foreground mb-5">
          Closed position breakdown · {PERIOD_LABEL[period]} · {s.totalTrades}{" "}
          trades
        </p>
        <div className="mb-5">
          <div className="flex justify-between text-xs text-muted-foreground mb-1.5">
            <span>
              {s.profitTrades} winning ({s.profitTradesPercent.toFixed(1)}%)
            </span>
            <span>
              {s.lossTrades} losing ({s.lossTradesPercent.toFixed(1)}%)
            </span>
          </div>
